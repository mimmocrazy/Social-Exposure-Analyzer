...
Poi passa ai social, come Instagram. I social moderni hanno difese anti-bot spietate basate sull'IA. Noi aggiriamo il "Login Wall" con una tecnica di *Impersonation*: preleviamo i cookie di sessione autenticata dal frontend e li iniettiamo negli header delle nostre richieste HTTP in Python. Meta crede di parlare con un browser umano e ci lascia estrarre l'albero delle amicizie e i post privati.
Tuttavia, le API possono accorgersi dell'inganno. Se Meta ci blocca restituendo un `403 Forbidden`, la nostra infrastruttura non va mai in *Hard Crash*. Subentra il pattern di **Graceful Degradation** (Degrado Controllato): il sistema intercetta l'errore, svuota preventivamente i cookie e riesegue l'attacco in modalità 'Ospite Pubblico', assicurandosi di portare a casa quantomeno i dati basilari (come la bio) per completare l'audit."
