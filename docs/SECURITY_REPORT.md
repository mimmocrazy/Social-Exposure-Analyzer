...
| 09 | `auth.py` | A07:2021 - Identification and Authentication Failures | È stato introdotto un bypass dell'autenticazione JWT per velocizzare il testing locale, iniettando un utente di default (`local_admin@local.host`). Se esposto in cloud, causerebbe un accesso amministrativo non autorizzato globale. | Critico | Implementato gating tramite flag d'ambiente o logica isolata in locale. Necessaria rimozione o segregazione per l'ambiente di produzione. | Aperto / Warning |
| 10 | `scraper.py` | A04:2021 - Insecure Design (Session Hijacking / Exposure) | L'integrazione del Deep Scan di Facebook richiede l'iniezione dei cookie `c_user` e `xs` inviati dal client. Rischio di logging involontario o furto del token di sessione. | Alto | I cookie vengono mantenuti solo nel context in-memory di `httpx` e non vengono mai scritti a log né salvati nel database. L'input utente nel frontend (React) avviene su connessione idealmente HTTPS. | Risolto |
| 11 | `risk_engine.py` | A05:2021 - Security Misconfiguration (Rate Limit DoS) | L'utilizzo eccessivo del free-tier di API LLM (Gemini) porta a fallimenti continui (HTTP 429) e possibile Denial of Service sull'applicativo. | Alto | Implementata un'architettura di HA con key-rotation, fallback automatico a 3-tier (GitHub Models -> Gemini -> Groq) e marcatura inibitoria temporizzata (60s) per i modelli esauriti. | Risolto |
| 12 | `.env` | A05:2021 - Security Misconfiguration (Secret Leak) | Salvataggio accidentale o persistenza di Personal Access Token di GitHub nel repository. | Critico | Il file `.env` è rigorosamente coperto dal `.gitignore`. L'accesso a `GITHUB_TOKEN` avviene dinamicamente a runtime senza hardcoding nel sorgente Python. | Risolto |
