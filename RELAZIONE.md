...
5. **Autenticazione e Modello SaaS (Multi-Tenancy):** Sebbene attualmente il frontend operi in modalità "Demo Pubblica", l'architettura backend è stata ingegnerizzata nativamente (tramite librerie di crittografia Bcrypt, token JWT e schemi Pydantic dedicati) per supportare un sistema Multi-Tenant. Questo sviluppo è fondamentale per la transizione in ambiente Enterprise, permettendo di istituire un sistema di *Rate-Limiting* a protezione del budget API, garantire una rigida *Segregazione dei Dati* della cronologia per singolo account, e mantenere un *Audit Trail* legale e tracciabile in ottemperanza ai principi di Accountability previsti dal GDPR.
