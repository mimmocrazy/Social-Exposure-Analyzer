...
5. **Autenticazione e Gestione Utenti Commerciale:** Sebbene attualmente il sito web operi come "Demo Pubblica", le fondamenta del backend sono già state programmate (tramite crittografia delle password e database utenti) per supportare il login. Questo sviluppo sarà fondamentale se il progetto diventerà un prodotto commerciale vero e proprio, permettendo di raggiungere tre obiettivi essenziali: (A) inserire un limite massimo di scansioni giornaliere per evitare che un utente (o un bot) consumi tutto il budget a pagamento delle API di Intelligenza Artificiale; (B) garantire la privacy totale, facendo in modo che ogni utente possa vedere solo la propria cronologia delle analisi e non quella degli altri; (C) sapere sempre con certezza chi ha effettuato una specifica ricerca, un requisito legale fondamentale nel campo della sicurezza informatica per rintracciare eventuali abusi o casi di stalking a tutela del GDPR.
